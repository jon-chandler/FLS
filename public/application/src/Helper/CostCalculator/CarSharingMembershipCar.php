 ...
 * Membership Car
 */
class CarSharingMembershipCar extends CostCalculator
{
    /**
    ...
    {
        $vehicleCosts = [];
        $runningCosts = $this->con->fetchAssoc('SELECT parking_home, membership_sharing_fee FROM insurance_price WHERE insurance_group = ?', [$vehicle['InsuranceGroup']]);

        $parkingHome = null;
        $membershipSharingFee = null;

        if ($runningCosts) {
            $parkingHome = (float) $runningCosts['parking_home'];
            $membershipSharingFee = (float) $runningCosts['membership_sharing_fee'];
        }
        ...
        if ($membershipSharingFee) {
            $cost = $membershipSharingFee;
            $vehicleCost = new VehicleCost();
