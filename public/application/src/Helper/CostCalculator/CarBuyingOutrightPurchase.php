 ...
 * Outright Purchase
 */
class CarBuyingOutrightPurchase extends CostCalculator
{
    /**
    ...
    public function calculateCosts(array $vehicle, array $answers, array $cacheValues = []): array
    {
        ...
        if ($this->isSnapshotAnswers) {
            ...
            if (array_key_exists('wouldYouConsiderSharingYourCarOrDriveway', $tempAnswers)) {
                $wouldYouConsiderSharing = $tempAnswers['wouldYouConsiderSharingYourCarOrDriveway'][0]->getOption()->getOptionTitle();
                $showIncome = (strtolower($wouldYouConsiderSharing) === 'yes') ? true : false;
            }
        }
        ...
        $vehicleCost = new VehicleCost();
        $vehicleCost->setFrequencyTitle(VehicleCost::FREQUENCY_TITLE_PAID_OVER_TERM)
            ->setCategory(VehicleCost::CAT_NET_VEHICLE_PROVIDER)
            ->setIsIncludedInTotal(false)
            ->setIsIncludedInCatTotal(false);
        $vehicleCosts[] = $vehicleCost;
        ...
        $vehicleCost = new VehicleCost();
        $vehicleCost->setFrequencyTitle(VehicleCost::FREQUENCY_TITLE_MONTHLY_COST)
            ->setCategory(VehicleCost::CAT_NET_VEHICLE_PROVIDER)
            ->setIsIncludedInTotal(false)
            ->setIsIncludedInCatTotal(false);
        $vehicleCosts[] = $vehicleCost;
